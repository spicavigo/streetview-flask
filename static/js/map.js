window.gdrive = (function(){
    ...
    function populateImages(points){
        ...
        for(i=1; i<points.length; i++){
            ...
            point_bearing.push([points[i-1].lat(), points[i-1].lng(), bearing]);
    ...
    function ajaxVideo(points){
        $.get("/get_video", {'data[]': points}, function(data){
            $('.download').show()
            $('#download-video').attr('href', data);
        });
    }
    var gdrive = {
        init: init,
